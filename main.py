...
def main():
    def initial_options():
        ...
        st.session_state.user_menu = streamlit_option_menu.option_menu(
            menu_title='How can I assist your movie search? 👀',
            options=['Recommend me a similar movie', 'Get movie details', 'Browse all movies'],
        ...
        elif st.session_state.user_menu == 'Get movie details':
    ...
    def recommend_display():
        ...
        st.title('Cine-Match: Your Movie Companion')

        selected_movie_name = st.selectbox(
            'Pick a movie to explore...', new_df['title'].values
        )

        rec_button = st.button('Recommend')
        if rec_button:
            st.session_state.selected_movie_name = selected_movie_name
            recommendation_tags(new_df, selected_movie_name, r'Files/similarity_tags_tags.pkl',"for You")
            recommendation_tags(new_df, selected_movie_name, r'Files/similarity_tags_genres.pkl',"Based on Genre")
            recommendation_tags(new_df, selected_movie_name,
                                r'Files/similarity_tags_tprduction_comp.pkl',"from the Same Production Company")
            recommendation_tags(new_df, selected_movie_name, r'Files/similarity_tags_keywords.pkl',"Based on Keywords")
            recommendation_tags(new_df, selected_movie_name, r'Files/similarity_tags_tcast.pkl',"Based on Cast")
        ...
        with st.container():
            ...
            with text_col:
                ...
                with col1:
                    ...
                    st.text(info[4])
                with col2:
                    st.text("Budget (in millions)")
                    st.text(info[1]/1000000)
                with col3:
                    st.text("Revenue (in millions)")
                    st.text(info[5]/1000000)
